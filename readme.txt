...
* The wide panel fits without scrolling on most phones: the photograph is
  contained in a shorter box instead of filling a square one. Where it still
  does not fit, the panel fades at the edge and nudges once on opening — and
  where it does fit, neither happens.
...
* Everything the storefront downloads is now minified: half the bytes for the
  same behaviour. The initial script is 1.3 KB gzipped and the player chunk
  7.6 KB.

= 0.9.3 =
* A video uploaded through a phone link arrived with no cover image and no
  size, so the shop drew an empty circle — which looks exactly like a video
  that never arrived. Both ways of uploading now finish through the same
  code.
* A phone that cannot compress video in the browser still sends a cover image
  and the video's size.
